  ...
  let reslut = await page.evaluate(function (){
    ...
    resources.each((index, el) => {
      ...
      apis_el.each((i, e) => {
        ...
        let name = (pathSplit[len - 3] || '').replace(/-/g,"") + (function (name){return name.charAt(0).toUpperCase() + name.slice(1)})(pathSplit[len - 2]).replace(/-/g,"") + (function (name){return name.charAt(0).toUpperCase() + name.slice(1)})(pathSplit[len - 1]).replace(/-/g,"");
